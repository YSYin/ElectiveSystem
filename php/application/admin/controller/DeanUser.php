...
class DeanUser extends Base {

    /**
     * 查询并返回教务用户列表，并提供相应的操作接口
     */
    ...
    public function addUser() {
        if (request()->isPost()) {
            ...
            $res = model('AdminUser')->addUser($data, 'Dean');
            if ($res){
                return json(['status' => 1, 'msg' => "您已成功添加教务".$data['user_name']]);
            } else {
                Base::addLog(2, $err);
                return json(['status' => 0, 'msg' => '添加教务用户失败']);
            }
        }
        else {
    ...
    public function checkGetUserName($user_name) {
        ...
        $exist = Base::checkUserAndRole($user_name, 'Dean');
        if ($exist == -1) {
    ...
    public function editUserInfo() {
        ...
        if (request()->isPost()) {
            ...
            if ($res) {
                ...
                return "<script>alert('教务信息并未变化');window.history.go(-1);</script>";
            }
        }
        else {
            $this->view->engine->layout("window");
