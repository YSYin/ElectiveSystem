...
func processElection(election CourseElection) {
	student_id := election.StudentID
	course_ids := election.CourseIDs

	db, err := sql.Open("mysql", "web_admin:WEB_ADMIN-2020/web_elective_db?charset=utf8")
...
func recvFromQueue() {
	conn, err := amqp.Dial("amqp://web_admin:WEB_ADMIN-2020@localhost:5672/")
